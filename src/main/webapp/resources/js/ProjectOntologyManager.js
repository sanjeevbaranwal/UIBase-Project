...
function populateAllTreeNodeData() {
	//alert('working ... name is - ' + nodeName);
	//alert(myurl);
	var project = document.getElementById("projectVal").value;
	localStorage.setItem( "projectVal",project);
...
function renderChildrens(obj, pnode, childs) {
	for (var i = 0; i < childs.length; i++) {
		cnode = pnode.createChildNode(obj[childs[i]]["text"], false,
...
function showDebitButton(){
	...
	function clicksubmitfunction()
	{
		...
		$.ajax({
				url: myurl+"updateOntologyClass/"+project,
				type: 'POST',        
	...
	function EditEntityfunction(id) {
		//window.location.href="WEB-INF/pages/jsp/ontologymanager.jsp";
		//setScreenValuesOntologyManager();
		
		//var newtext='';
		var str=id;
		//alert(id);
		var tempStr = "EntityEditimage";
		str = str.substr(tempStr.length, str.length);
		// str = str.slice(str.length-1, str.length);
		var EditEntityNo="entityNumber"+str;
		//alert(EditEntityNo);
		var EditEntityNumber=$("#"+EditEntityNo).text();
		//alert(EditEntityNumber);
		var project = document.getElementById("projectVal").value;
		var tempURL = myurl+"getQueAttributesByQueName/"+EditEntityNumber+","+project;
	//	alert(tempURL);
			$.ajax({
				url: tempURL,
				type: 'POST',        
	...
	//	alert(tempURL);
			$.ajax({
				...
				//alert(response);
					...
					$('#questiondescrption_id').val(response.questionVariations);
					//$('#answer_id').val(response.answerDescription); 
					$('#answer_id').val("ResponseKey-"+selectedCategory+"-"); 
					isExistingQuestion = true;
				},
				error : function(xhr, status, error) {
					//alert(xhr.responseText+"inside error");
...
function addArticle() {
	...
	var jbpmfilepath= $('#jbpmfilename_id').val();
	...
	var answerDescription = $('#answer_id').val().trim();
	...
	//alert(JSON.stringify(json));
	$.ajax({
		url : myurl+"modifyOntologyNode/"+project,
		type : 'POST',
		data : JSON.stringify(json),
		cache : false,
		beforeSend : function(xhr) {
		...
		success : function(response) {
			...
			/* var msgToShow = '';
			if (response == 'Success') {
				msgToShow = "Data Updtaed Successfully!"
			} else {
				msgToShow = "Unable to Save Data!"
			}
			$("#dialog").dialog('open');
			ChangeMessage(msgToShow);
			AutoCloseDialogBox(4000); */
			window.location.href = "projectontologymanager";
		},
		error : function(xhr, status, error) {
			$('#answer_id').val('');
			$('#newque_id').val('');
...
function deleteEntityfunction(id) {
	//window.location.href="WEB-INF/pages/jsp/ontologymanager.jsp";
	//setScreenValuesOntologyManager();
	...
	var project = document.getElementById("projectVal").value;
	//alert(EditEntityNumber);
	var tempURL = myurl+"removeEntityNode/"+EditEntityNumber+","+project;;
//	alert(tempURL);
		$.ajax({
			url: tempURL,
			type: 'POST',        
			...
			success:function(response){ 
			alert("Question node deleted successfully");
			window.location.href = "projectontologymanager";	
			},
			error : function(xhr, status, error) {
				//alert(xhr.responseText+"inside error");
...
function AddCategorysubmitfunction(){
		...
		//alert(json);
//alert(JSON.stringify(json));		
	//var tempURL = myurl+'getNounVerbMandatoryAssertion/';
	$.ajax({
		url : myurl+"addNewClassInOntology/"+project,
		type : 'POST',
		data : JSON.stringify(json),
		cache : false,
		beforeSend : function(xhr) {
...
{
	document.location.href=getContextPath()+"/downloadExcel";
}
